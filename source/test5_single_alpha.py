...
def dualoptimization(ptsI,calib_net,sfm_net,shape_gt=None,fgt=None,M=100,N=68,mode='still',ptstart=0):
    ...
    for outerloop in itertools.count():
        ...
        for iter in itertools.count():
            opt1.zero_grad()
            f = calib_net(ptsI) + 300
            f_mu = f.mean().detach().item()
            f_std = f.std().detach().item()
            f = f.mean()
            ...
            # apply loss
            loss = torch.mean(error2d * alpha.squeeze() - torch.log(alpha.squeeze()))
            ...
            print(f"iter: {iter} | error: {loss.item():.3f} | f/fgt: {f.item():.1f}/{ftrue:.1f} | error2d: {error2d.mean().item():.3f} | error_time: {error_time.item():.2f} | rmse: {rmse:.2f} | alpha_mu: {alpha.mean().item():.2f} | alpha_std: {alpha.std().item():.2f} | fmu: {f_mu:.2f} | fstd: {f_std:.3f}")
        ...
        for iter in itertools.count():
            ...
            if not fgt is None:
                ...
                fgt = -1
            f_error = torch.mean(torch.abs(f-ftrue))
            print(f"iter: {iter} | error: {loss.item():.3f} | f/fgt: {f.item():.1f}/{ftrue:.1f} | error2d: {error2d.mean().item():.3f} | error_time: {error_time.item():.2f} | rmse: {rmse:.2f} | alpha_mu: {alpha.mean().item():.2f} | alpha_std: {alpha.std().item():.2f}")
