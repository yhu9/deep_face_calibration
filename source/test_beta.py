...
def test(modelin=args.model,outfile=args.out,feature_transform=args.feat_trans):
    ...
    for f_test in f_vals:
        ...
        for j, data in enumerate(loader):
            ...
            error_rel3d = []
            error_relf = []
            for beta_count in [i*10 for i in range(1,21)]:
                if beta_count == 200: beta_count = 199
                # create a model and optimizer for it
                model1 = Model1(k=beta_count,feature_transform=False)
                model1.apply(util.init_weights)
                model2 = Model1(k=1, feature_transform=False)
                model2.apply(util.init_weights)
                opt1 = torch.optim.Adam(model1.parameters(),lr=1e-1)
                opt2 = torch.optim.Adam(model2.parameters(),lr=1)
                ...
                for outerloop in itertools.count():
                    ...
                    for iter2 in itertools.count():
                        ...
                        loss.backward()
                        opt2.step()
                        print(f"iter2: {iter2} | error: {loss.item():.3f} | f/fgt: {f.item():.1f}/{fgt[0].item():.1f} | rmse: {rmse.item():.2f}")
                        if iter2 == 20: break
                    ...
                    for iter1 in itertools.count():
                        ...
                        # shape prediction via 3dmm
                        betas,_,_ = model1(x.unsqueeze(0))
                        shape = torch.sum(betas*lm_eigenvec[:,:beta_count],1)
                        ...
                        loss.backward()
                        opt1.step()
                        print(f"iter1: {iter1} | error: {loss.item():.3f} | f/fgt: {f.item():.1f}/{fgt[0].item():.1f} | rmse: {rmse.item():.2f}")
                        if iter1 == 20: break
            ...
            matdata = {}
            matdata['beta_n'] = np.array(beta_n)
            matdata['error_reld'] = np.array(error_rel3d)
            matdata['error_relf'] = np.array(error_relf)
            out = f"beta_analysis/seq{j:03d}_{outfile}"
            scipy.io.savemat(out,matdata)
