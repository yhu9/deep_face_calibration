...
def test(modelin=args.model,outfile=args.out,optimize=args.opt):
    ...
    for f_test in f_vals:
        ...
        for j,data in enumerate(loader):
            ...
            orig = ptsI.unsqueeze(0).permute(0,2,1,3)
            ...
            # save delta info
            delta = adj_net(orig).detach()
            x = orig + delta
            x_img_gt = x_img_gt.permute(1,0,2)

            #print(orig.squeeze() - x_img_gt)
            error_orig = torch.mean(torch.norm(orig.squeeze() - x_img_gt,dim=0))
            error_delta = torch.mean(torch.norm(x.squeeze() - x_img_gt,dim=0))
            orig_error.append(error_orig.item())
            delta_error.append(error_delta.item())
            ...
            if optimize:
                ...
                opt3 = torch.optim.Adam(adj_net.parameters(),lr=1e-1)
    ...
    matdata['seterror_rel3d'] = np.array(seterror_rel3d)
    matdata['seterror_relf'] = np.array(seterror_relf)
    matdata['error_orig'] = np.array(orig_error)
    matdata['error_delta'] = np.array(delta_error)
    scipy.io.savemat(outfile,matdata)
